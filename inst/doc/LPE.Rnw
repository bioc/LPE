...
conditions. To use it for paired data, see \Rpackage{LPEP} library. For using
LPE in multiple conditions, use \Rpackage{HEM} library.
...
small subsetion of Ley data (1000 rows) in the subsequent examples.
...
interquartile ranges on all chips are set to their widest range),
thresholding (making the instensity values lower than 1.0 to 1.0),
log based 2 transformation and LOWESS normalization (if LOWESS is
set to TRUE).  Note that this preprocess is a simple constant-scale and
location-normalization step.
...
replicates calculated from the \code{baseOlig.error} function, and Gene
IDs as probe.set.name.
...
\code{z.critical} corresponding to 5\%, (=1.77), and subselect all the genes
obtained from LPE-method (section \ref{z-stats}) for which
